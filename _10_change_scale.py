...
def scale_test():
    ...
    first_cube.paint_uniform_color([1,0,0])
    first_cube.compute_vertex_normals()
    cubes.append(first_cube)

    second_cube = copy.deepcopy(first_cube)
    second_cube.translate([0.25,1,0])
    second_cube.scale(-2, center=second_cube.get_center())

    cubes.append(second_cube)
...
def translate_test():
    ...
    second_cube = copy.deepcopy(first_cube)
    second_cube.paint_uniform_color([0,1,0])
    second_cube.translate([7, 7, 0])
    # second_cube.scale(-2, center=second_cube.get_center())
    first_cube.translate(second_cube.get_center())
...
desired_num_branches = 1

tree_height = total_num_cubes / desired_num_branches

# theta = (np.pi * 2) / 500
# theta = (np.pi * 2) / tree_height * 0.5
theta = 0
# theta = np.pi / 4
...
while cube_counter < cube_limit:
    ...
    for top_cube_i in range(len(temp_top_cubes)):
        ...
        new_cube_mesh = o3d.geometry.TriangleMesh.create_box(width=running_scale,
                                                        height=running_scale,
                                                        depth=running_scale)

        running_scale -= scale_change_rate

        new_cube_mesh.translate(top_cube.mesh.get_center())
        new_cube = Shape(new_cube_mesh,
                         top_cube.rotation,
                         top_cube.branch_length,
                         1)

        # new_cube.size *= scale_change_range


        # new_cube.mesh.scale(1- scale_change_range, center=new_cube.mesh.get_center())
        # print(new_cube.size)
        ...
        if place_on_x_axis:
            # scaled_x =  (x_default * 0.5 * scaler_modifier) + (x_default * 0.5 * scaler_modifier)
