...
def change_colors_on_merged_mesh():
        ...
        second_cube.translate([1,0,0])
        second_cube.paint_uniform_color([0,1,0])

        first_cube += second_cube
        cubes.append(first_cube)

        third_cube = copy.deepcopy(second_cube)
        third_cube.translate([-1.5,0.5,0.5])
        third_cube.paint_uniform_color([0,0,1])
        cubes.append(third_cube)


        print(first_cube.is_intersecting(third_cube))
...
theta = np.pi / tree_height * 0.5
theta = np.pi / tree_height * 0.2
# theta = 0
# theta = np.pi / 4

# branch_len = num_cubes/ 16
branch_len = 100

hue_increment = 4 / branch_len
...
while cube_counter < cube_limit:
    ...
    for top_cube_i in range(len(temp_top_cubes)):
        ...
        # this seems to work and i have no idea why.
        ...
        if only_x_axis:
            ...
            if new_cube.mesh.is_intersecting(out_cube):
                # actual_num_branches -= 1
                continue
            ...
            if random.random() < branch_prob:
                ...
                if branch_cube.mesh.is_intersecting(out_cube):
                    # actual_num_branches -= 1
                    continue
