...
def tapered(base_width, end_width, length, gap, angle,offset_x, offset_y, left, right, center, with_corner):
    ...
    for i in range(len(points)):
        points[i] = (
            ...
            points[i][0] + (offset_x * (1.5 * 3 + 3 * 2)),
            points[i][1] + (offset_y * (1.5 * 3 + 3 * 2))
        )
    return points
...
def gen_test_pieces():
    doc = ezdxf.new()
    # doc = ezdxf.new()
    # ezdxf.options.support_dirs = "C:\jj_laptop_files\Python Projects\20251103     cad dxf\fonts"
    # ezdxf.options.support_dirs = "C:/jj_laptop_files/Python Projects/20251103     cad dxf/fonts"
...
def export(doc):
    ...
    l = 100
    page = layout.Page(150,20, layout.Units.cm, margins=layout.Margins.all(l * 0.04))
